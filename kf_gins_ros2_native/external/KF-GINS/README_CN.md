...
### 2.4 Sage-Husa 自适应观测噪声 (R)

KF-GINS 支持基于 Sage-Husa 思路的创新序列(innovation)在线估计 GNSS 位置观测噪声协方差 $R$，用于 GNSS 位置 STD 不稳定或随环境变化时的自适应调节。

- 自适应更新作用于 GNSS 位置观测的噪声阵 $R$（北/东/地三轴）。
- 为了稳健性，默认只更新 $R$ 的对角线，并对方差设置下限，避免滤波器过度“自信”。

**推荐默认组合（稳健、通用）：**

在配置文件中加入如下可选段（不写则使用程序默认值）：

```yaml
sage_husa:
	enable: true
	alpha: 0.95            # 遗忘因子 (0,1)，越大越平滑/收敛越慢
	diag_only: true        # 仅更新对角线（推荐）
	min_var_factor: 0.1    # 每轴方差下限 = min_var_factor * R0
	min_var_abs: 0.0       # 绝对方差下限 (单位^2)，0 表示不额外启用
```

**参数说明：**

- `alpha`：常用范围 0.95~0.99；想更快跟随环境变化可适当调小。
- `diag_only`：仅在你确实需要估计“全矩阵(带相关项)的 R”时再设为 `false`；程序会对其做 SPD 投影。
- `min_var_factor` / `min_var_abs`：防止 $R$ 退化过小，提升数值稳定性。
